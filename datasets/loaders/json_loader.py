...
def load_dataset(name: str, split: str = "train", index_col: str = "date") -> pd.DataFrame:
    ...
    file_path = os.path.join(data_dir, name, f"{split}.json")
    ...
    try:
        ...
        logger.info(f"✅ 加载 JSON 数据集: {file_path}")
        return df
    except Exception as e:
        logger.exception(f"📛 加载 JSON 失败: {file_path}")
        raise e
